...
export class ServerService {
    ...
    public url = 'http://localhost:8983/solr/';
    public collection = 'articlescollection';
    public mode = '/select?q=';
    public query: string;
    public pattern = /([\!\*\+\-\=\<\>\&\|\(\)\[\]\{\}\^\~\?\:\\/"])/g;
    ...
    findJournals(mytitle: string, myabstract: string) {
        ...
        this.search = this.url + this.collection + this.mode + this.query + this.default_field + this.settings;
